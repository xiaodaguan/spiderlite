 ...
 * @date 2018/4/17
 */
@Component(value = "zhipinCompFetcher")
@Slf4j
public class CompanyFetcher extends BaseFetcher {


    private static final String URL_TEMPLATE = "https://m.zhipin.com{compHref}";
    ...
    public void fetch(Task task, ICallBack callBack) {
        String compHref = Optional.of(task.getCtx()).map(ctx -> ctx.get("compHref")).map(String::valueOf).orElse("");

        String url = URL_TEMPLATE.replace("{compHref}", compHref);
